...
| cornell | 康奈尔笔记 | 活页本风格，左线索词右内容，底部总结 | 复习回顾 |
...
- **英文斜体**：JS `wrapEnglish()` 遍历文本节点，仅将 `[a-zA-Z0-9]+` 包裹为 `<span class="en">`，CSS `.en { font-style: italic }`，跳过 `.katex`
...
3. **字体规范（强制）**：汉字使用 Microsoft YaHei（正体），英文和数字使用 Times New Roman（斜体），公式使用 KaTeX 默认字体。CSS 写法：`font-family:'Times New Roman','Microsoft YaHei',serif`，配合 JS `wrapEnglish()` 实现英文斜体。禁止使用 `font-mono` 覆盖数字字体。
4. **不要出现"AI 生成"字样**
...
10. **层级结构**：sections 使用 level 1/2/3 + children 嵌套，cues 仅在 level 1 节点使用（康奈尔左侧线索栏）
